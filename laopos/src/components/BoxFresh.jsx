...
const Box = (props) => {
  ...
  const acceptOrder = async (session, id) => {
    return await axios
      .post(
        "http://192.168.1.112:4000/api/acceptOrder",
        {
          crossDomain: true,
          data: {
            session: session,
  ...
  return (
    <div className="box" style={{ backgroundColor: "#C3E3C5" }}>
      ...
      <div className="btn">
        <Link
          ...
          className="btnaccept"
        >
          <i className="fa fa-solid fa-check" aria-hidden="true"></i>Accept
        </Link>
        <Link to="#" className="btnreject">
          <i className="fa fa-solid fa-xmark" aria-hidden="true"></i>Reject
        </Link>
      </div>
    </div>
  );
